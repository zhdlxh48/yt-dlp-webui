...
class RecorderService:
    ...
    async def start_live(self, channel_ids: list[str] | None = None) -> list[JobInfo]:
        ...
        if settings.tools.auto_install_tools:
            tool_status = await self.tools.install_missing()
        else:
        ...
        for channel in selected:
            ...
            jobs.append(await self._start_job("live", title, command))
        if not jobs:
    ...
    async def _start_job(self, kind: str, title: str, command: list[str]) -> JobInfo:
        job_id = uuid4().hex
        job = JobInfo(
            id=job_id,
